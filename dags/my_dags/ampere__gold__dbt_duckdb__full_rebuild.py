...
DAG_ID = "ampere__gold__dbt_duckdb__full_rebuild"
...
with DAG(
    ...
    start_task = PythonOperator(
        task_id="run__gold__full_rebuild__start",
        python_callable=print,
        op_args=["##### startGoldFullRebuild #####"],
    )

    run_gold_dbt = KubernetesPodOperator(
        task_id="run__gold__dbt_full_rebuild",
        name="ampere-dbt-gold-full-rebuild",
        ...
        env_vars={
            ...
            "THREADS": Variable.get("gold_full_rebuild_dbt_threads", default="2"),
            "DUCKDB_MEMORY_LIMIT": Variable.get(
                "gold_full_rebuild_duckdb_memory_limit",
                default=DAG_CONFIG.full_rebuild_duckdb_memory_limit,
            ),
        ...
        container_resources=V1ResourceRequirements(
            requests={
                "cpu": Variable.get(
                    "gold_full_rebuild_dbt_cpu_request",
                    default=DAG_CONFIG.full_rebuild_cpu_request,
                ),
                "memory": Variable.get(
                    "gold_full_rebuild_dbt_memory_request",
                    default=DAG_CONFIG.full_rebuild_memory_request,
                ),
            },
            limits={
                "cpu": Variable.get(
                    "gold_full_rebuild_dbt_cpu_limit",
                    default=DAG_CONFIG.full_rebuild_cpu_limit,
                ),
                "memory": Variable.get(
                    "gold_full_rebuild_dbt_memory_limit",
                    default=DAG_CONFIG.full_rebuild_memory_limit,
                ),
            },
        ),
        get_logs=True,
    ...
    done_task = PythonOperator(
        task_id="run__gold__full_rebuild__done",
        python_callable=print,
        op_args=["##### doneGoldFullRebuild #####"],
    )
