...
def get_sa_engine_from_conn(conn_id: str):
    c = BaseHook.get_connection(conn_id)
    extra = c.extra_dejson or {}
    params = {
        "driver": extra.get("driver", "ODBC Driver 18 for SQL Server"),
        "Encrypt": extra.get("Encrypt", "no"),
        "TrustServerCertificate": extra.get("TrustServerCertificate", "yes"),
    }
    uri = (
        f"mssql+pyodbc://{quote_plus(c.login)}:{quote_plus(c.password)}@"
        f"{c.host}:{c.port or 1433}/{c.schema}?{urlencode(params)}"
    )
    return create_engine(uri, pool_pre_ping=True, pool_recycle=600)
    ...
    def export_chunk(prefix: str, chunk: dict) -> str | None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with engine.connect().execution_options(
                stream_results=True, yield_per=10_000
            ) as conn:
                df = pl.read_database(query=sql, connection=conn)
