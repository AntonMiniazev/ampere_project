# Get products from Products.csv
# Take random products from each category
# Assign result to store from Store.csv
# cols: [product_id, store_id]

import pandas as pd
import numpy as np
import os

script_dir = os.path.dirname(os.path.abspath(__file__))
product_path = os.path.join(script_dir, "Product.csv")
store_path = os.path.join(script_dir, "Store.csv")

# Read Product and Store tables
df_products = pd.read_csv(product_path)
df_store = pd.read_csv(store_path)

num_of_categories = len(df_products['category_id'].unique())

df_assortment = pd.DataFrame()


for idx in df_store['id']:
    for cat_id in range(1, num_of_categories + 1):
        category_len = len(df_products[df_products['category_id'] == cat_id])
        mask = np.zeros(category_len, dtype=int) # generating zeros for all products
        num_ones = np.random.randint(1, 4) # 1-3 products won't be in assortment from each category
        ones_indices = np.random.choice(category_len, size=num_ones, replace=False)
        mask[ones_indices] = 1 # 1 to be filtered as products not in assortment

        # Selecting category with random number of products and assign to a store
        df_assortment = pd.concat([
            df_assortment,
            df_products[df_products['category_id'] == cat_id]
                .loc[mask == 0]
                .assign(store_id=idx)[['id', 'store_id']]
        ], ignore_index=True)

df_assortment.rename(columns={"id": "product_id"})
df_assortment.to_csv(os.path.join(script_dir, 'assortment.csv'), index=False,sep=",")
