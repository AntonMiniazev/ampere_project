...
if [ "$(hostname)" = "ampere-t-k8s-master" ]; then
  ...
  kubectl get ns ampere-project-t >/dev/null 2>&1 || kubectl create ns ampere-project-t
  ...
  helm upgrade --install minio . --namespace ampere-project-t --create-namespace
